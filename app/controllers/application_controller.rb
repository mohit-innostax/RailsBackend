class ApplicationController < ActionController::Base
  ...
  def authorize_request
    ...
    if token.blank?
      redirect_to "/old-user", allow_other_host: true, alert: "Session expired. Please log in again." and return
    end
    begin
      decode_the_token= JWT.decode(token, SECRET_KEY, true, algorithm: "HS256")[0]
      ...
      cookies.delete(:jwt)
      redirect_to "http://localhost:3000/old-user", allow_other_host: true and return
    end
  end
  def refresh_token
    if @user
